...
public class ClientResource {
     ...
     */
    @GetMapping("/clients")
    public List<Client> getAllClients(@RequestParam(required = false) String filter,@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        ...
        if ("hirer-is-null".equals(filter)) {
            log.debug("REST request to get all Clients where hirer is null");
            return StreamSupport
                .stream(clientRepository.findAll().spliterator(), false)
                .filter(client -> client.getHirer() == null)
                .collect(Collectors.toList());
        }
