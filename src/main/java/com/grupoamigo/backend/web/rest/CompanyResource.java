...
public class CompanyResource {
    ...
    public List<Company> getAllCompanies(@RequestParam(required = false) String filter,@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        if ("membership-is-null".equals(filter)) {
            log.debug("REST request to get all Companys where membership is null");
            return StreamSupport
                .stream(companyRepository.findAll().spliterator(), false)
                .filter(company -> company.getMembership() == null)
