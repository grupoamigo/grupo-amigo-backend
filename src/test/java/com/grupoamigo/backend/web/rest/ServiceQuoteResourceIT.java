...
public class ServiceQuoteResourceIT {
    ...
    @Mock
    private ServiceQuoteRepository serviceQuoteRepositoryMock;
    ...
    public void setup() {
        ...
        final ServiceQuoteResource serviceQuoteResource = new ServiceQuoteResource(serviceQuoteRepository);
    ...
    public void getAllServiceQuotesWithEagerRelationshipsIsEnabled() throws Exception {
        ServiceQuoteResource serviceQuoteResource = new ServiceQuoteResource(serviceQuoteRepositoryMock);
        when(serviceQuoteRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restServiceQuoteMockMvc = MockMvcBuilders.standaloneSetup(serviceQuoteResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
            .setMessageConverters(jacksonMessageConverter).build();

        restServiceQuoteMockMvc.perform(get("/api/service-quotes?eagerload=true"))
        .andExpect(status().isOk());

        verify(serviceQuoteRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({"unchecked"})
    public void getAllServiceQuotesWithEagerRelationshipsIsNotEnabled() throws Exception {
        ServiceQuoteResource serviceQuoteResource = new ServiceQuoteResource(serviceQuoteRepositoryMock);
            when(serviceQuoteRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
            MockMvc restServiceQuoteMockMvc = MockMvcBuilders.standaloneSetup(serviceQuoteResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
